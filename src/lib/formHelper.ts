    ...
    .map((field) => {
      ...
      if (shadcn) {
        if (field.type === "radio" && field.options) {
          ...
          fieldCode = `${field.name}: radio buttons with options ${radioOptions}, ${validationRules}, ${labelCode}. Use <RadioGroup> and <RadioGroupItem>${
            rhf
              ? " with <Controller> and include onValueChange={field.onChange} and defaultValue={field.value} in <RadioGroupItem>"
              : ""
          }.`;
